# Gemini Notebook 转写下载器 / Gemini Notebook Transcript Downloader

> 一个极简的 Chrome 扩展：在 Gemini Notebook 音频来源的文字转写旁加一个按钮，
> 一键把转写导出成排版整洁的 Markdown（去除中文字间空格、繁体转简体、按说话轮次分段）。
>
> A minimal Chrome extension that adds a button next to a Gemini Notebook audio
...
在新的 `notebook.google.com` 上生效，也保留对旧的 `notebooklm.google.com` 域名的兼容。当你打开某个音频来源的文字转写后，
**来源标题（如 `xxx.m4a`）旁边**会出现一个蓝色的 **Download .md** 按钮。
点击即可把转写保存为 Markdown 文件，并自动完成以下处理：

1. **去除字间空格** —— Gemini Notebook 会在每两个中文字之间插入一个空格，这里会把它们去掉；
   中文标点前后的空格也会去掉（例如 `OK 。` → `OK。`）。英文单词、数字两侧的空格会保留
   （例如 `用 Python 写代码` 保持不变）。
2. **繁体转简体** —— 通过内置的 OpenCC 离线完成繁体（台湾）到简体的转换。
3. **按说话轮次分段** —— Gemini Notebook 用「双空格」标记不同说话轮次的边界。脚本会先按这些
...
5. 打开一个 Gemini Notebook 笔记本，点击某个音频来源，让它的文字转写显示出来。
6. 点击来源标题旁边的蓝色 **Download .md** 按钮。

### 文件说明

- `manifest.json` —— Manifest V3，仅作用于 Gemini Notebook（并兼容旧 NotebookLM 域名）。
...
- Gemini Notebook 是一个 Angular 应用，内部属性 ID（如 `_ngcontent-ng-c4155300573`）会随版本变化，
...
Works on the new `notebook.google.com` domain and remains compatible with the
legacy `notebooklm.google.com` domain. When you open an audio source's text
...
1. **Space-stripped** — the space Gemini Notebook inserts between every two Chinese
...
3. **Split into paragraphs by turn** — Gemini Notebook marks the boundary between
...
5. Open a notebook on Gemini Notebook, click an audio source so its transcript shows.
6. Click the blue **Download .md** button next to the source title.

### Files

- `manifest.json` — Manifest V3, scoped to Gemini Notebook and its legacy domain.
...
- Gemini Notebook is an Angular app whose internal attribute IDs
